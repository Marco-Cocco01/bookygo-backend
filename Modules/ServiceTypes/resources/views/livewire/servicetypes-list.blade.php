            ...
            <div class="card custom-card">
                <div class="card-header">
                    <div class="card-title">
                        <a class="btn btn-primary" href="{{ route('service-type.add') }}">Aggiungi Nuovo</a> 
                    </div>
                </div>
                <div class="card-body">
                    <table id="responsiveDataTable" class="table table-bordered text-wrap w-100">
                        ...
                        <tbody>
                            @forelse($serviceTypes as $key => $val)
                            <tr>
                                <td>{{ $val['title'] }}</td>
                                ...
                                <td>
                                    <a href="{{ route('service-type.edit', $val['id']) }}"><i class="bi bi-pencil-square"></i></a>
                                    <a href="#" wire:click="confirmDelete({{ $val['id'] }})" class="ms-2"><i class="bi bi-trash"></i></a>
                                </td>
                            </tr>
                            @empty
                                <tr class="text-center">
                                    <td colspan="7" class="text-center">Nessuna tipologia di servizio trovata</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
