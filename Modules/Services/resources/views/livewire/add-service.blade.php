            ...
            <div class="card custom-card">
                ...
                <div class="card-body">
                   <form wire:submit=@if($is_edit)"update" @else "add" @endif>
                        ...
                        <div class="row mb-3">
                            <label for="cell" class="col-sm-2 col-form-label">Tipologia di servizio</label>
                            <div class="col-sm-10">
                                <select class="form-select w-50" aria-label="Default select example" wire:model="id_service_type">
                                    <option value="" >Seleziona</option>
                                    @foreach($serviceTypes as $key => $val)
                                        <option value="{{ $val['id'] }}">
                                           {{ $val['category']['name'] }} | {{ $val['title'] }} 
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <button type="submit" class="btn btn-primary mt-5">@if($is_edit)Modifica @else Aggiungi @endif servizio </button>
                    </form>
